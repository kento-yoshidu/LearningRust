...
fn input_category_type(register_type: u8) -> u8 {
	...
	let category_type: u8 = category_type.trim().parse().expect("カテゴリーは数値で入力して下さい。");
...
fn input_price() -> u32 {
	...
	price.trim().parse().expect("日付はYYYY-MM-DDの形式で入力してください。")
}
...
fn input_date() -> NaiveDate {
	...
	NaiveDate::from_str(&date).expect("日付の形式が不正です。")
}
